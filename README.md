...
- **Fault**: 以下を優先度順に判定し、検知したらバルブを強制的に閉じます（ヒステリシス・デバウンス付き）。
  ...
  センサ異常時は、バナー・警告表示およびシリアルログに `(P1 P2)` のように原因センサ（P1=1次側 / P2=2次側 / FUEL=燃圧）が付記され、どのセンサが異常かを個別に識別できます。
...
4. LCD画面には1次側・2次側・燃圧（ゲージ付き）・バルブ状態・警告バナーがリアルタイム表示される。  
   燃圧の表示色は燃圧センサ自体の有効性・目標帯との比較のみで決まり、P1/P2側の異常（Fault遷移）による影響は受けない。
