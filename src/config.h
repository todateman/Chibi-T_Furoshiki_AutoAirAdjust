...
// 実機計測(2026-09-19, RIGOL DHO804/VISA-MCP, ソレノイドバルブ CKD USB2-M5-1-0-DC12V, 1次側約0.54MPa,
//   下流は運用時と同じ構成。docs/oscilloscope_dho804_visa_mcp.md参照。旧バルブ(airtac 2V025-08FT)の値は2026-08-11計測):
//   デッドタイム(駆動開始→2次側センサ電圧の有意な立ち上がり) 実測 4.1〜4.5ms(旧 約5.4ms)。
//     これはバルブの動き出しの遅れではなく、センサに圧力が届くまでの遅れを含む値。
//   最小開弁幅: 実測の開弁時間 3.28ms以下ではバルブが動かずΔP=0、3.65ms以上で動作(ΔP +0.047MPa以上)。
//     3.3〜3.65msの間でΔPが0→0.047MPaに急峻に立ち上がる(傾き約0.13MPa/ms)。
//   1パルスのΔP(開始圧0.23〜0.29MPa): 4ms → +0.055〜0.064MPa、8ms → +0.096(開始圧0.31)〜+0.15MPa(開始圧0付近)
//   整定時間(駆動停止→2次側センサ電圧が最終値の±0.01MPa以内に収束) 3.6〜4msパルス 15〜25ms、8msパルス(開始圧0付近)で最大34ms
//   閉弁のばらつき: 指令に対する実測開弁時間の増加は通常+0.08ms、27ショット中10回が+0.1ms超、3回が+0.4ms超(最大+0.86ms)
constexpr uint32_t PULSE_WIDTH_MIN_MS = 4;   // 下限パルス幅(動作境目の実測3.28〜3.65msに対し、指令4msで0.35ms以上の余裕。3.6ms以下は境目に近く不採用。旧8ms)
...
constexpr uint32_t PULSE_COOLDOWN_MS = 50; // パルス間の最小休止時間(実測整定時間 最大34ms < 50ms で十分なマージンを確保)
