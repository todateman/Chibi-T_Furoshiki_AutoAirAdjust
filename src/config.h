...
//
// !!! 重要 !!!
// KiCad回路図上ではソレノイド駆動信号 SOL_TRG は M-BUS 25番ピン
// (ネットラベル "GPIO34") に配線されているが、ESP32のGPIO34は
// ハードウェア的に入力専用ピンであり digitalWrite() で駆動できない。
// そのため実機では基板を改造し、下記 SOLENOID_PIN 宛にジャンパ線で
// 配線し直す必要がある。
//
// 暫定値として GPIO26 を指定しているが、これは「M5Core2内部で
// TFT/SDカード等に使用されていないと消去法で推測される」候補に
// 過ぎず、実機での導通確認・テスターでの検証を行うまでは確定情報
// ではない。ジャンパ配線が確定したら、この1箇所を書き換えること。
constexpr uint8_t SOLENOID_PIN = 26;
